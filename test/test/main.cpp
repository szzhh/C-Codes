...
void evolution()
{
	...
	for (int i = 0; i < int(population.size()); i++) {
		for (int j = 0; j < int(population.size()) - i - 1; j++) {
		...
		p_c_p.insert(p_c_p.begin(), population[int(population.size()) - 1 - i].c_p);
		add_c_p += 1 / p_c_p[0];
	}
	//每代的繁衍 
	for (int generation = 0; generation < max_generation; generation++) {
		int S = int(population.size());
		//繁衍过程 
		...
		if ((generation + 1) % die_generation == 0) {
			population.erase(population.begin() + int(population.size()) - int(population.size()*die_rate), population.begin() + int(population.size()));
			p_c_p.erase(p_c_p.begin() + int(p_c_p.size()) - int(p_c_p.size()*die_rate), p_c_p.begin() + int(p_c_p.size()));
		}
		for (int i = 0; i < int(p_c_p.size()); i++) {
			add_c_p += 1 / p_c_p[i];
		}
	...
	//最后的输出函数 
	for (int i = 0; i <int(population.size()); i++) {
		cout<<"逆波兰式为：" << population[i].code << ' ' <<"适应度为："<< population[i].c_p -0.8<< endl;
	}
	cout <<"最终进化的代数为："<< population.size() << endl;
...
void test::on_pushButton_clicked()
{
	...
	ui->label_14->setText(tempStr2.setNum(population[0].c_p-0.8));
}
...
{
	//Tree* tree = population[0].transform();
	QCoreApplication::setAttribute(Qt::AA_EnableHighDpiScaling);
	QApplication a(argc, argv);
	test w;
	w.show();
	//delete[](population[0].transform()->root);
